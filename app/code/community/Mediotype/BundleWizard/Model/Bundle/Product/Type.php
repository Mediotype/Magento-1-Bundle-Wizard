 ...
 * Time: 5:36 PM
 */
class Mediotype_BundleWizard_Model_Bundle_Product_Type extends Mage_Catalog_Model_Product_Type_Abstract
{
    protected function _prepareBuyRequest(Varien_Object $buyRequest, $product, $processMode)
    {
        $buyRequestData = $buyRequest->getData();
        $isDoubleSidedPost = false;

        Mage::log($buyRequestData, null, "MyLog.log");


        // Custom for ReflectAddress.com, checks for double sided posts
        foreach ($buyRequestData['bundle_option'] as $optionId => $selections) {

            $optionModel = $this->getOptionsByIds(array($optionId), $product)->getFirstItem();

            //Mage::getModel('bundle/option')->load($optionId);
            if (!$optionModel->getId()) {
                throw new Exception('Invalid option id');
            }

            if (stristr($optionModel->getTitle(), "post") !== false) {
                $selectionCollection = $this->getSelectionsCollection(array($optionId), $product);
                foreach ($selectionCollection as $selectionModel) {

                    if ($selectionModel->getSelectionId() == $buyRequestData['bundle_option'][$optionId] &&
                        stristr($selectionModel->getName(), 'double') !== false
                    ) {
                        $isDoubleSidedPost = true;
                        break;
                    }
                }
            }
        ...
        // PROCESS TEXT MAP INPUT
        foreach ($buyRequestData['bundle_option'] as $optionId => $selections) {

            $optionModel = Mage::getModel('bundle/option')->load($optionId);
            if (!$optionModel->getId()) {
                throw new Exception('Invalid option id');
            }

            if ($optionModel->getData('type') == 'textbox') {
                $newSelectionData = array();
                $newQtys = array();

                $selectionData = trim($selections[0]);
                $possibleSelections = $this->getSelectionsCollection(array($optionId), $product);
                $strLength = strlen($selectionData);
                $product->addCustomOption('selection_text_' . $optionModel->getId(), $selectionData);

                // MAP CHARACTERS IN THE TEXT INPUT TO PRODUCTS
                for ($i = 0; $i < $strLength; $i++) {
                    $character = substr($selectionData, $i, 1);
                    foreach ($possibleSelections as $selectionModel) {

                        if ($optionModel->getData('configurable_attribute_id') != 0) {
                            // IF THE OPTION IS CONFIGURABLE
                            if ($selectionModel->getData('textbox_attribute_value') == $character &&
                                $buyRequestData['bundle_option_config'][$optionId] == $selectionModel->getData('configurable_value_id')
                            ) {

                                $selectionId = $selectionModel->getData("selection_id");

                                $newSelectionData[$selectionId] = $selectionId;
                                if (!array_key_exists($selectionId, $newQtys)) {
                                    $newQtys[$selectionId] = 0;
                                }

                                //Custom for Reflect Address
                                if ($isDoubleSidedPost) {
                                    $newQtys[$selectionId] += 2;
                                } else {
                                    $newQtys[$selectionId]++;
                ...
                $buyRequestData['bundle_option'][$optionId] = $newSelectionData;
                $buyRequestData['bundle_option_qty'][$optionId] = $newQtys;
            }

        }

        // PROCESS AUTO ADD INPUT
        foreach ($buyRequestData['bundle_option'] as $optionId => $selections) {
            $optionModel = Mage::getModel('bundle/option')->load($optionId);
            if (!$optionModel->getId()) {
                throw new Exception('Invalid option id');
            ...
            if ($optionModel->getData('type') == 'autoadd' &&
                array_key_exists($optionModel->getData("autoadd_target_option"), $buyRequestData['bundle_option'])
            ) {

                $newSelectionData = array();
                $newQtys = array();
                $targetOption = Mage::getModel('bundle/option')->load($optionModel->getData("autoadd_target_option"));
                $autoAddSelections = $this->getSelectionsCollection(array($optionId), $product);
                $qty = 0;
                foreach ($autoAddSelections as $selectionIndex => $selectionModel) {
                    $selectionId = $selectionModel->getData("selection_id");

                    if ($targetOption->getType() == "textbox") {
                        // AGGREGATE QTY's
                        foreach ($buyRequestData['bundle_option_qty'][$targetOption->getId()] as $qtyPart) {
                            $qty += $qtyPart;
                        }

                    } else {
                        $qty += $buyRequestData['bundle_option_qty'][$targetOption->getId()];
                    }

                    $newSelectionData[$selectionId] = $selectionId;
                    $newQtys[$selectionId] = $qty;
                }
    ...
    {
        // ADDED _prepareBuyRequest to top of this function
        $this->_prepareBuyRequest($buyRequest, $product, $processMode);
            ...
            if (!$this->getProduct($product)->getSkipCheckRequiredOption() && $isStrictProcessMode) {
                foreach ($optionsCollection->getItems() as $option) {
                    if ($option->getRequired() && !isset($options[$option->getId()])) {
                        return Mage::helper('bundle')->__('Required options are not selected. ' . $option->getTitle());
                    }
                }
            }
            $selectionIds = array();
        ...
        if (count($selections) > 0 || !$isStrictProcessMode) {
            ...
            foreach ($selections as $selection) {
                ...
                // ADDED elseif statement to handle qty's to top of this function
                if ($selection->getSelectionCanChangeQty() && isset($qtys[$selection->getOptionId()])) {
                    ...
                    isset($qtys[$selection->getOptionId()]) &&
                    is_array($qtys[$selection->getOptionId()]) &&
                    isset($qtys[$selection->getOptionId()][$selection->getSelectionId()])
                ) {
                    $qty = $qtys[$selection->getOptionId()][$selection->getSelectionId()];
                } else {
    ...
    /**
     * Due to the inheritance chain the the modifications needed
     * FUNCTIONS BELOW THIS COMMENT ARE DIRECT COPIES FROM Mage_Bundle_Model_Product_Type
     */
